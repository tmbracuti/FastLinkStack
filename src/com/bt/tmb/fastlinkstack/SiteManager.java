...
public class SiteManager 
{
    ...
    public char publish(LinkBoundMessage msg) throws LinkPublishException
    {        
        LinkConnector target = null;
        if(m_conn1.isLinkConnected())
        {
            System.err.println("published to channel A");
            target = m_conn1;
        }
        else if(m_conn2.isLinkConnected())
        {
            System.err.println("published to channel B");
            target = m_conn2;
        }
        if(target == null)
        {
            throw new LinkPublishException("no valid ITS Link connections");
        }
        
        return target.publishToLink(msg);                
    }
